...
**Class relatedness (Python/TypeScript/Kotlin only).** A file containing 2 or more classes is flagged when those classes split into multiple families with nothing connecting them — no shared inheritance, no method signature referencing another class in the file, and no shared naming affix. Unlike the checks above, there's no minimum class count before this can fire: a class is already a much stronger unit of cohesion than a single function (a whole type, not one operation), so even 2 completely unrelated classes are worth flagging.
...
This check says nothing about a single class carrying too much responsibility by itself (a "god class") - that's a distinct concern, deliberately not covered here.
