...
**Class methods are grouped by their enclosing class, not counted as free-standing functions.** Every signal below that operates on "functions" (function-count sprawl and its type-cohesion check) only ever sees module-level, free-standing functions — a class's methods are excluded and evaluated instead by the class-relatedness check further down. This matters for OOP-style files (Python/TypeScript/Kotlin/C++/C#; F# has no class construct this detector models): without it, a file of two small, tightly related classes with 15 methods total between them reads exactly like 15 unrelated free-standing functions, and the type-cohesion signal — which only sees each method's own explicitly-annotated parameter/return types, not the fact that the method belongs to one of only two classes — misreads the handful of incidental types (mostly the classes' own names, showing up in constructors and factory-style methods) as "unrelated types" sprawl. A class's *own* method count and responsibility are judged separately by the god-class check further down — see [God class](god-class.md), which flags one type whose methods span too many unrelated domains rather than counting classes or free-standing functions.

**Directly bound anonymous callables follow the same responsibility boundaries.** An anonymous callable assigned directly to a module-level name counts as a free-standing function. One assigned directly to a class member counts as a method in languages whose classes this detector models. Function-local callable variables, returned closures, and inline callbacks do not add file or class responsibilities; their enclosing callable already represents that implementation boundary. Binding names feed the naming-cohesion heuristic, and explicit anonymous-callable parameter types feed type cohesion under the same coverage requirement as named functions. Recognition is intentionally syntax-only: aliases and assignments through intermediate expressions are not resolved. C# has no modeled module-level anonymous-callable binding form, and F# class members remain outside class-relatedness because the detector does not model an F# class construct.

**Function-count sprawl.** A file with more than 12 functions is flagged (medium; high past 15). The threshold drops to 8 if the filename itself contains `util`, `helper`, or `common`, treating the name as a proxy for "already known to be a grab-bag." These three bars (`genericFunctionCount`/`highFunctionCount`/`utilsFileFunctionCount`) are all configurable — see [Configuration](#configuration). The violation message deliberately steers away from "just split this file" — since the detector only sees one file, it can't tell whether a split would land the code in an existing cohesive module or just distribute the same coupling across new files with more imports wiring them back together.
...
**Import member fan-out.** Ten or more explicitly imported declarations from one source are flagged, even when that source counts as only one dependency. Kotlin uses a fixed threshold of 30 because one-declaration-per-import syntax is idiomatic for large external APIs. The configurable threshold applies to other languages. This signal can also arise from Python `from` imports or TypeScript named imports. The message asks whether the file is an intentional composition boundary; it does not prescribe a split or a wildcard import.
...
- `energyStateAnalyzer.coherence.utilsFileFunctionCount` (default `8`), function count above which a file whose name contains `util`, `helper`, or `common` is flagged for function-count sprawl. Raising it relaxes the utils-file check; lowering it tightens it.
- `energyStateAnalyzer.coherence.genericFunctionCount` (default `12`), function count above which any file is flagged for function-count sprawl. Raising it relaxes the check; lowering it tightens it.
- `energyStateAnalyzer.coherence.highFunctionCount` (default `15`), function count above which function-count sprawl is reported at high severity instead of medium.
- `energyStateAnalyzer.coherence.largeFunctionSeverityMultiplier` (default `1.5`), multiplier applied to `maxLargeFunctions`; a file with more large functions than this times the bar is flagged at high severity instead of medium (e.g. 5 × 1.5 = 7.5).

The function-count sprawl thresholds are configurable: `utilsFileFunctionCount` (8 for utils/helper/common-named files), `genericFunctionCount` (12 generic), and `highFunctionCount` (15, the high-severity cutoff); plus `largeFunctionSeverityMultiplier` (1.5) on the large-function severity bar. The import signals are configurable via `siblingOpenThreshold`, `importBreadthThreshold`/`highImportBreadthThreshold` (distinct-module breadth and its high-severity cutoff), and `memberImportFanOutThreshold` for non-Kotlin languages; Kotlin uses its fixed 30-declaration bar. Wildcard imports are structural findings and need no count threshold.
