...
The function-count sprawl thresholds themselves (8 for utils-named files, 12 generic, 15 for high severity), dependency-breadth thresholds (10/15), member-fan-out threshold (10 bindings from one source), and F# sibling-open threshold (5 modules) are fixed heuristics, not exposed as settings. Wildcard imports are structural findings and need no count threshold.
