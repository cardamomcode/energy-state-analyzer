...
- The `fsharp-analyzers` dotnet tool (0.39.0) is restored by the existing `dotnet tool restore` step; run it locally with `just fsharp-analyze [paths]`.
