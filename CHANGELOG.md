---
last_commit_released: c0a6f5a8c518f40f6b98d2641aee901a3cf52182
name: energy-state-analyzer
updaters:
  - command: npm version {version} --no-git-tag-version --allow-same-version
  - regex:
      file: src/Core/ReportSarif.fs
      pattern: (?<=let private toolVersion = ").*(?=")
---

# Change Log

All notable changes to the "energy-state-analyzer" extension are generated by
[EasyBuild.ShipIt](https://github.com/easybuild-org/EasyBuild.ShipIt) from
[Conventional Commits](https://www.conventionalcommits.org/).

## 0.21.0 - 2026-09-21

### 🏗️ Breaking changes

* Default scan report is JSON; files lists findings only (#163) ([91dbb4c](https://github.com/cardamomcode/energy-state-analyzer/commit/91dbb4c715193548aeb01508ae0d5f966738472d))

### 🐞 Bug Fixes

* Exclude the following function's doc comment from F# function line counts (#164) ([6eeb8bb](https://github.com/cardamomcode/energy-state-analyzer/commit/6eeb8bb1f491135f50c4444d9213e9dc6ff767d0))

<strong><small>[View changes on Github](https://github.com/cardamomcode/energy-state-analyzer/compare/edb9fcbccaedfcfed2c16e16b83980406df943c1..c0a6f5a8c518f40f6b98d2641aee901a3cf52182)</small></strong>

## 0.20.0 - 2026-09-20

### 🚀 Features

* Flag boolean validators in parse-dont-validate (#159) ([edb9fcb](https://github.com/cardamomcode/energy-state-analyzer/commit/edb9fcbccaedfcfed2c16e16b83980406df943c1))

<strong><small>[View changes on Github](https://github.com/cardamomcode/energy-state-analyzer/compare/c6607b76a5705d83e19bc3198c7f012a6c34692c..edb9fcbccaedfcfed2c16e16b83980406df943c1)</small></strong>

## 0.19.1 - 2026-09-17

### 🐞 Bug Fixes

* Give boolean blindness a targeted primitive-obsession message (#156) ([bafb1ce](https://github.com/cardamomcode/energy-state-analyzer/commit/bafb1cef928b09870583d59e31207ecd2b22e96b))
* Add typed format specifiers to detector messages (#158) ([c6607b7](https://github.com/cardamomcode/energy-state-analyzer/commit/c6607b76a5705d83e19bc3198c7f012a6c34692c))

<strong><small>[View changes on Github](https://github.com/cardamomcode/energy-state-analyzer/compare/d638fb6b75608731fb0ff06d12717a1a0c9be442..c6607b76a5705d83e19bc3198c7f012a6c34692c)</small></strong>

## 0.19.0 - 2026-09-16

### 🚀 Features

* Introduce anonymous callable model (#153) ([c3129f3](https://github.com/cardamomcode/energy-state-analyzer/commit/c3129f3427247d32d056bb6035e0efbd55017bea))
* Integrate bound callables with coherence (#155) ([d638fb6](https://github.com/cardamomcode/energy-state-analyzer/commit/d638fb6b75608731fb0ff06d12717a1a0c9be442))

<strong><small>[View changes on Github](https://github.com/cardamomcode/energy-state-analyzer/compare/00fd3e6fbca7cbfa1fec8417ffe5ee805b0960c5..d638fb6b75608731fb0ff06d12717a1a0c9be442)</small></strong>

## 0.18.1 - 2026-09-15

### 🐞 Bug Fixes

* Relax Kotlin import member fan-out threshold (#148) ([d69b16a](https://github.com/cardamomcode/energy-state-analyzer/commit/d69b16a55d38a66aa37795ed21d82d840205e8e7))

<strong><small>[View changes on Github](https://github.com/cardamomcode/energy-state-analyzer/compare/9cff149fd78949a772a402cc26bb4c29cf6857fe..00fd3e6fbca7cbfa1fec8417ffe5ee805b0960c5)</small></strong>

## 0.18.0 - 2026-09-14

### 🚀 Features

* Separate broad protected scope, recovery dominance, and oversized recovery blocks (#142) ([26564a3](https://github.com/cardamomcode/energy-state-analyzer/commit/26564a3e89d15fbe2abc0f4caf200fe48fb7392b))

### 🐞 Bug Fixes

* Make inversion feedback respect control flow (#136) ([271676f](https://github.com/cardamomcode/energy-state-analyzer/commit/271676f2043dd7fbc8c9ff593e859dd6c6b1c5da))
* Clarify energy-state model and report interpretation (#140) ([f2c9ae6](https://github.com/cardamomcode/energy-state-analyzer/commit/f2c9ae6879f76220d72b9a72b5afc53b142a6ca2))
* Report tool version in SARIF output (#141) ([ad611cf](https://github.com/cardamomcode/energy-state-analyzer/commit/ad611cf7f7d24b6f9daeb9e8bfb8413b585b6a40))
* Ordinal StartsWith comparison + watch task matcher (#145) ([9cff149](https://github.com/cardamomcode/energy-state-analyzer/commit/9cff149fd78949a772a402cc26bb4c29cf6857fe))

<strong><small>[View changes on Github](https://github.com/cardamomcode/energy-state-analyzer/compare/94105c06c3a96d2ab1bce15729fb26a64da92154..9cff149fd78949a772a402cc26bb4c29cf6857fe)</small></strong>

## 0.17.1 - 2026-09-13

### 🐞 Bug Fixes

* Align cognitive complexity scoring with documented rules (#132) ([7b28561](https://github.com/cardamomcode/energy-state-analyzer/commit/7b285618630793b753c660887fc8078b9d784fb7))
* Use explicit StringComparison overload in C# adapter (#134) ([94105c0](https://github.com/cardamomcode/energy-state-analyzer/commit/94105c06c3a96d2ab1bce15729fb26a64da92154))

<strong><small>[View changes on Github](https://github.com/cardamomcode/energy-state-analyzer/compare/d3b62b03378b0ef2e9b31391515866e9c0a602d9..94105c06c3a96d2ab1bce15729fb26a64da92154)</small></strong>

## 0.17.0 - 2026-09-13

### 🚀 Features

* Detect discarded validation guarantees (#128) ([d3b62b0](https://github.com/cardamomcode/energy-state-analyzer/commit/d3b62b03378b0ef2e9b31391515866e9c0a602d9))

<strong><small>[View changes on Github](https://github.com/cardamomcode/energy-state-analyzer/compare/0da9a473c2dafc34fc94df424a3bca7015855b33..d3b62b03378b0ef2e9b31391515866e9c0a602d9)</small></strong>

## 0.16.0 - 2026-09-11

### 🚀 Features

* Add C# analysis support (#125) ([0da9a47](https://github.com/cardamomcode/energy-state-analyzer/commit/0da9a473c2dafc34fc94df424a3bca7015855b33))

<strong><small>[View changes on Github](https://github.com/cardamomcode/energy-state-analyzer/compare/98c73c11364c3abe5c7e52ffb5c2288ed7f4815d..0da9a473c2dafc34fc94df424a3bca7015855b33)</small></strong>

## 0.15.3 - 2026-09-11

### 🐞 Bug Fixes

* Stabilize editor analysis and clean release packages (#123) ([98c73c1](https://github.com/cardamomcode/energy-state-analyzer/commit/98c73c11364c3abe5c7e52ffb5c2288ed7f4815d))

<strong><small>[View changes on Github](https://github.com/cardamomcode/energy-state-analyzer/compare/abe97d47a5e9000e1d8aa82bd51c9c47cbdceaf6..98c73c11364c3abe5c7e52ffb5c2288ed7f4815d)</small></strong>

## 0.15.2 - 2026-09-08

### 🐞 Bug Fixes

* Clarify error boundary scope analysis (#116) ([efc3d7c](https://github.com/cardamomcode/energy-state-analyzer/commit/efc3d7c7ffdce40274e957081352bd3f55b3af9f))
* Use project config for analyzer thresholds (#119) ([abe97d4](https://github.com/cardamomcode/energy-state-analyzer/commit/abe97d47a5e9000e1d8aa82bd51c9c47cbdceaf6))

<strong><small>[View changes on Github](https://github.com/cardamomcode/energy-state-analyzer/compare/7097fc14feb3918d098e28f259d57006433b1df1..abe97d47a5e9000e1d8aa82bd51c9c47cbdceaf6)</small></strong>

## 0.15.1 - 2026-09-07

### 🐞 Bug Fixes

* Correct F# and-bindings, postfix types, named args, and string matches (#112) ([99380dd](https://github.com/cardamomcode/energy-state-analyzer/commit/99380ddd870517969797649a99f75e447b73c326))

<strong><small>[View changes on Github](https://github.com/cardamomcode/energy-state-analyzer/compare/b685a15415160c1ff9dcf6f0505b19a70e52db2b..7097fc14feb3918d098e28f259d57006433b1df1)</small></strong>

## 0.15.0 - 2026-09-06

### 🚀 Features

* *(sarif)* Add stable IDs and VS Code export (#99) ([97049bf](https://github.com/cardamomcode/energy-state-analyzer/commit/97049bf1a498da8b4917d009d7849b085b7645a4))

### 🐞 Bug Fixes

* Prefix command titles with plugin name (#102) ([e25eb9a](https://github.com/cardamomcode/energy-state-analyzer/commit/e25eb9a009e5c70e6174ffc589fb413512694101))
* Specify report URI suffix comparison (#104) ([da8d22f](https://github.com/cardamomcode/energy-state-analyzer/commit/da8d22fede5b4103ff478c9c4d3b0a971a6f408d))
* *(ci)* Correct security-events permission key (#103) ([181b310](https://github.com/cardamomcode/energy-state-analyzer/commit/181b310cc32dfd865147b861962583022e1dd6c9))
* *(sarif)* Link rules to detector guides (#101) ([4087dee](https://github.com/cardamomcode/energy-state-analyzer/commit/4087deeab082a4890abd28aed9233369be0b24de))

<strong><small>[View changes on Github](https://github.com/cardamomcode/energy-state-analyzer/compare/eec025fc61c7d60083567d605817aa38ba64d314..b685a15415160c1ff9dcf6f0505b19a70e52db2b)</small></strong>

## 0.14.0 - 2026-09-06

### 🚀 Features

* *(cli)* Default reports to SARIF (#98) ([eec025f](https://github.com/cardamomcode/energy-state-analyzer/commit/eec025fc61c7d60083567d605817aa38ba64d314))

### 🐞 Bug Fixes

* Skip generated directories during scans (#96) ([f8b14e9](https://github.com/cardamomcode/energy-state-analyzer/commit/f8b14e91932a676526bd00cc924bf46830bd8def))

<strong><small>[View changes on Github](https://github.com/cardamomcode/energy-state-analyzer/compare/9d5839162703674ed718d973853e3398d3e13da4..eec025fc61c7d60083567d605817aa38ba64d314)</small></strong>

## 0.13.0 - 2026-09-04

### 🚀 Features

* Add error shadowing detector (#91) ([312a9b3](https://github.com/cardamomcode/energy-state-analyzer/commit/312a9b35f0ccc4d360401bfb960b91468f396e71))

### 🐞 Bug Fixes

* Exclude nested functions from error shadowing (#93) ([9d58391](https://github.com/cardamomcode/energy-state-analyzer/commit/9d5839162703674ed718d973853e3398d3e13da4))
* *(detector)* Honor god-class static and count exemptions (#94) ([c7b8f7d](https://github.com/cardamomcode/energy-state-analyzer/commit/c7b8f7d68900312efca917103e46dd90e234f7a0))

<strong><small>[View changes on Github](https://github.com/cardamomcode/energy-state-analyzer/compare/f21dcbf98bde0c68ffe935b91e74432e39c26977..9d5839162703674ed718d973853e3398d3e13da4)</small></strong>

## 0.12.0 - 2026-09-04

### 🚀 Features

* Add per-detector enable/disable toggles and color settings (#90) ([f21dcbf](https://github.com/cardamomcode/energy-state-analyzer/commit/f21dcbf98bde0c68ffe935b91e74432e39c26977))
* *(core)* Add god-class detector to file-coherence check (#80) ([0da71cd](https://github.com/cardamomcode/energy-state-analyzer/commit/0da71cd1b518f1f2f8adea1e9f5613f5eddcbdc5))

### 🐞 Bug Fixes

* Resolve all fsharp-analyzers findings across F# source (#84) ([34892de](https://github.com/cardamomcode/energy-state-analyzer/commit/34892de4f18a9ae0564a004ed8668e74bcac5bd4))
* Reduce remaining analyzer findings in TypeCohesion and Python functions (#89) ([94f2c38](https://github.com/cardamomcode/energy-state-analyzer/commit/94f2c380df809a7972442883218dfca942f29587))
* *(analyzer)* Extract magic-number constants into named module-level bindings (#87) ([95b517c](https://github.com/cardamomcode/energy-state-analyzer/commit/95b517c0c559a3bb1753b3e4489672024d57e79e))
* *(ci)* Scan the default branch via schedule + manual trigger (#86) ([e213496](https://github.com/cardamomcode/energy-state-analyzer/commit/e2134966acb85beeb7bedd6cbd0b820d2e96e3b2))

<strong><small>[View changes on Github](https://github.com/cardamomcode/energy-state-analyzer/compare/7951a86146ecb0068b5d308f8a7b80134b819a4f..f21dcbf98bde0c68ffe935b91e74432e39c26977)</small></strong>

## 0.11.0 - 2026-09-03

### 🚀 Features

* Make Core.Config the single source of truth for thresholds and options (#74) ([42dcd1d](https://github.com/cardamomcode/energy-state-analyzer/commit/42dcd1d32c29a19fc16cba2c3d4860d0f42f74e3))
* *(core)* Make import-coherence thresholds configurable (#78) ([7951a86](https://github.com/cardamomcode/energy-state-analyzer/commit/7951a86146ecb0068b5d308f8a7b80134b819a4f))

<strong><small>[View changes on Github](https://github.com/cardamomcode/energy-state-analyzer/compare/b2795c89d9df95e46d26171a5393129c8f0208c3..7951a86146ecb0068b5d308f8a7b80134b819a4f)</small></strong>

## 0.10.1 - 2026-09-02

### 🐞 Bug Fixes

* *(extension)* Preserve Python editor findings (#72) ([b2795c8](https://github.com/cardamomcode/energy-state-analyzer/commit/b2795c89d9df95e46d26171a5393129c8f0208c3))

<strong><small>[View changes on Github](https://github.com/cardamomcode/energy-state-analyzer/compare/7879447ccd900a3aef6e14b44c1e48969e907e62..b2795c89d9df95e46d26171a5393129c8f0208c3)</small></strong>

## 0.10.0 - 2026-09-02

### 🚀 Features

* Distinguish import coherence signals (#70) ([7879447](https://github.com/cardamomcode/energy-state-analyzer/commit/7879447ccd900a3aef6e14b44c1e48969e907e62))

<strong><small>[View changes on Github](https://github.com/cardamomcode/energy-state-analyzer/compare/3aadd519fe3bb11fa9a79562a1c5b09035245568..7879447ccd900a3aef6e14b44c1e48969e907e62)</small></strong>

## 0.9.1 - 2026-09-01

### 🐞 Bug Fixes

* *(cognitive)* Stop scoring lambdas as decision points (#66) ([3aadd51](https://github.com/cardamomcode/energy-state-analyzer/commit/3aadd519fe3bb11fa9a79562a1c5b09035245568))

<strong><small>[View changes on Github](https://github.com/cardamomcode/energy-state-analyzer/compare/49ef35194407cbc9ca2f63e4248fd1683e03a3f6..3aadd519fe3bb11fa9a79562a1c5b09035245568)</small></strong>

## 0.9.0 - 2026-09-01

### 🚀 Features

* *(cpp)* Add C++ analysis support (#61) ([8a81808](https://github.com/cardamomcode/energy-state-analyzer/commit/8a818080a901b0a93d5b2c55ba75f30e36391133))

### 🐞 Bug Fixes

* *(ci)* Handle energy diff reports from forks (#63) ([3ac23aa](https://github.com/cardamomcode/energy-state-analyzer/commit/3ac23aa0f1167ec2d96aae7f75e5518587097c12))

<strong><small>[View changes on Github](https://github.com/cardamomcode/energy-state-analyzer/compare/db726199ca6214ebdf2da5fa0a73d4f29bf6be97..49ef35194407cbc9ca2f63e4248fd1683e03a3f6)</small></strong>

## 0.8.1 - 2026-08-31

### 🐞 Bug Fixes

* *(extension)* Preserve VS Code allowlist values (#59) ([db72619](https://github.com/cardamomcode/energy-state-analyzer/commit/db726199ca6214ebdf2da5fa0a73d4f29bf6be97))

<strong><small>[View changes on Github](https://github.com/cardamomcode/energy-state-analyzer/compare/01777cc27c9c08136661a2f1e21409cdf94f05ce..db726199ca6214ebdf2da5fa0a73d4f29bf6be97)</small></strong>

## 0.8.0 - 2026-08-31

### 🚀 Features

* Rewrite analyzer in F# for Fable (#46) ([8aaf666](https://github.com/cardamomcode/energy-state-analyzer/commit/8aaf6666c537b9f1655175805128749e56b8d933))
* *(core)* Calculate McCabe cyclomatic complexity (#55) ([9a42b6a](https://github.com/cardamomcode/energy-state-analyzer/commit/9a42b6a68cf94c7583c1d1a8282ddffa961275c7))
* *(core)* Add include-test-files toggle for magic detectors (#57) ([271e15c](https://github.com/cardamomcode/energy-state-analyzer/commit/271e15c8a914f550a5079789750a71caba34181d))
* *(extension)* Split src/extension.ts by domain (#30) (#42) ([b9f1ba3](https://github.com/cardamomcode/energy-state-analyzer/commit/b9f1ba37d93cbfd34275814a7bdbfb591cd9ae06))

### 🐞 Bug Fixes

* *(extension)* Construct VS Code diagnostics correctly (#54) ([c9b2f3f](https://github.com/cardamomcode/energy-state-analyzer/commit/c9b2f3f4a8c1c6550776bc1b1b8c611483fba7b3))
* *(suppressions)* Anchor esa-ignore directive regex so prose isn't matched (#45) ([c99b26a](https://github.com/cardamomcode/energy-state-analyzer/commit/c99b26ae425f16c16460b42d474dd52934f3610d))

<strong><small>[View changes on Github](https://github.com/cardamomcode/energy-state-analyzer/compare/5d63923aba3248095bda4074aff14bb133f4f65b..01777cc27c9c08136661a2f1e21409cdf94f05ce)</small></strong>

## 0.7.0 - 2026-08-28

### 🚀 Features

* Add includeFixtures setting for visually verifying detector fixtures (#40) ([5d63923](https://github.com/cardamomcode/energy-state-analyzer/commit/5d63923aba3248095bda4074aff14bb133f4f65b))

### 🐞 Bug Fixes

* Group class methods by enclosing type in file-coherence detector (#39) ([3274ad5](https://github.com/cardamomcode/energy-state-analyzer/commit/3274ad53d7e58a40c3a9ddae3c859e0cf9e7ee39))

<strong><small>[View changes on Github](https://github.com/cardamomcode/energy-state-analyzer/compare/ba8974593949c853eabeedd60da6b0bacd84d73c..5d63923aba3248095bda4074aff14bb133f4f65b)</small></strong>

## 0.6.0 - 2026-08-27

### 🚀 Features

* Add type-based cohesion signal to the file-coherence detector (#38) ([ba89745](https://github.com/cardamomcode/energy-state-analyzer/commit/ba8974593949c853eabeedd60da6b0bacd84d73c))

### 🐞 Bug Fixes

* Exempt annotated const val from Kotlin magic-number detector (#36) ([3439614](https://github.com/cardamomcode/energy-state-analyzer/commit/3439614b1ad4e8f2394e854c5bc8abbaab8c84ba))

<strong><small>[View changes on Github](https://github.com/cardamomcode/energy-state-analyzer/compare/c52e34266a2eaeedc6f15e737f593420f3e340f9..ba8974593949c853eabeedd60da6b0bacd84d73c)</small></strong>

## 0.5.0 - 2026-08-27

### 🚀 Features

* Exempt test files from the magic-number detector (#33) ([18023bb](https://github.com/cardamomcode/energy-state-analyzer/commit/18023bb92d74f66f189ac7207c1c9bb40f17aca9))

### 🐞 Bug Fixes

* Count distinct import sources, not raw lines, for import sprawl (#35) ([c52e342](https://github.com/cardamomcode/energy-state-analyzer/commit/c52e34266a2eaeedc6f15e737f593420f3e340f9))

<strong><small>[View changes on Github](https://github.com/cardamomcode/energy-state-analyzer/compare/e7c2d41d1347a4a10f7a921e077a45f745eade4c..c52e34266a2eaeedc6f15e737f593420f3e340f9)</small></strong>

## 0.4.0 - 2026-08-26

### 🚀 Features

* Add .esaignore to exclude paths from the analyzer (#29) ([bd526b4](https://github.com/cardamomcode/energy-state-analyzer/commit/bd526b45970c72d611e127e2ed7f111f2655f937))
* Inline esa-ignore suppression directives (#25) ([e7c2d41](https://github.com/cardamomcode/energy-state-analyzer/commit/e7c2d41d1347a4a10f7a921e077a45f745eade4c))

### 🐞 Bug Fixes

* Remove duplicate activation toasts on startup (#27) ([d144a00](https://github.com/cardamomcode/energy-state-analyzer/commit/d144a00770492252b50870b4cf3cebc1b44ba4c5))
* Count try/catch blocks toward nesting-depth violations (#28) ([d483c16](https://github.com/cardamomcode/energy-state-analyzer/commit/d483c165b4ba63a2e6a4c6eae5dfd62bc9868ecc))

<strong><small>[View changes on Github](https://github.com/cardamomcode/energy-state-analyzer/compare/489d195ffe125b4e3ebcb9905f561f2d94daf1ad..e7c2d41d1347a4a10f7a921e077a45f745eade4c)</small></strong>

## 0.3.0 - 2026-08-25

### 🚀 Features

* Add Kotlin support and lazy-load language grammars (#21) ([3a1078e](https://github.com/cardamomcode/energy-state-analyzer/commit/3a1078ecac31ea1574635cb01f794e767b098183))
* Repo scanning, PR baseline diffing, and a complexity-scored human report (#20) ([489d195](https://github.com/cardamomcode/energy-state-analyzer/commit/489d195ffe125b4e3ebcb9905f561f2d94daf1ad))

<strong><small>[View changes on Github](https://github.com/cardamomcode/energy-state-analyzer/compare/b3dda28dffdd3dc8c6cee2d5b07d5a99d50761cb..489d195ffe125b4e3ebcb9905f561f2d94daf1ad)</small></strong>

## 0.2.0 - 2026-08-25

### 🚀 Features

* Add opaque-boolean-literal detector (#16) ([d12447b](https://github.com/cardamomcode/energy-state-analyzer/commit/d12447bb07607c5fdc0b62781fb6df69ffbef1f4))

### 🐞 Bug Fixes

* Add braces to satisfy eslint curly rule (#10) ([8fd41cd](https://github.com/cardamomcode/energy-state-analyzer/commit/8fd41cde59103455a76ab1cbb504492578bbc3b3))
* Tailor primitive-obsession advice to each language (#7) ([cf7807b](https://github.com/cardamomcode/energy-state-analyzer/commit/cf7807b55de2efbb0befd8a6a8a282f3a33215f3))
* Cover primitive-obsession membership check with a test, correct heatmap color docs (#12) ([ba0050c](https://github.com/cardamomcode/energy-state-analyzer/commit/ba0050cb2b1894d0d90f7e80b273defa72f77277))
* Split magic-value detection into separate number/string detectors (#13) ([4aca127](https://github.com/cardamomcode/energy-state-analyzer/commit/4aca127af06ee4ea2d71a4ccb8d47d26ef22014e))
* Dogfood the extension's own CLI on its source, fix bugs it found (#14) ([bffadd3](https://github.com/cardamomcode/energy-state-analyzer/commit/bffadd3b2d50e700414ad313afdcc68be385125b))
* Suppress primitive-obsession swap-risk for Python keyword-only params (#15) ([eb1c592](https://github.com/cardamomcode/energy-state-analyzer/commit/eb1c59211823a6dd2dff152be8bd536531b4648a))
* Exempt Python module-level constants from magic-number detection (#17) ([0024607](https://github.com/cardamomcode/energy-state-analyzer/commit/0024607849eb0d15e5d5c5925a5e2b804b49616f))

<strong><small>[View changes on Github](https://github.com/cardamomcode/energy-state-analyzer/compare/0cbd44479c8048e4313fae49fcb157a52900d5f6..b3dda28dffdd3dc8c6cee2d5b07d5a99d50761cb)</small></strong>

## 0.1.2 - 2026-08-24

### 🐞 Bug Fixes

* Restore shebang in bundled dist/cli.js ([8b54f00](https://github.com/cardamomcode/energy-state-analyzer/commit/8b54f00269b555b7743cbffb648e411142496efe))

<strong><small>[View changes on Github](https://github.com/cardamomcode/energy-state-analyzer/compare/94e05303b32730a02656ebd820f356b50d49b59b..0cbd44479c8048e4313fae49fcb157a52900d5f6)</small></strong>

## [0.1.1]

- Added the missing `license` field to `package.json` (MIT) — npm was displaying the package as "Proprietary".

## [0.1.0]

This entry marks the release baseline used when adopting ShipIt. Entries below
this point were written by hand.

## [0.0.10]

- Fixed the F# magic-value detector: any literal inside a `let`-bound function body was being treated as a named constant, because F#'s grammar gives function definitions and plain value bindings the same node type. Only the direct value of a plain (non-function) `let` binding is now exempted.
- Added `energyStateAnalyzer.magicValues.enabled` (default `true`) to toggle the magic-value detector on/off.

## [0.0.8]

- Extended the primitive obsession detector to F# and TypeScript (previously Python only): consecutive same-typed primitive parameters and variables compared against 3+ distinct string literals are now flagged in all three languages. Python retains an additional `in (a, b, c)` membership-check variant that F#/TypeScript have no direct grammar equivalent for.

## [0.0.7]

- Added a primitive obsession detector (Python only): flags consecutive same-typed primitive parameters (e.g. `lat: float, lon: float`, a swap risk) and variables compared against 3+ distinct string literals (a de facto enum encoded as strings).

## [0.0.6]

- Added a file coherence check that flags files with too many large functions (configurable via `energyStateAnalyzer.coherence.largeFunctionLines` and `.maxLargeFunctions`), independent of total function count, so languages like F# with many small functions per module aren't penalized.
- Initial release
