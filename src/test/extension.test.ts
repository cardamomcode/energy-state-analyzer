...
suite('analyzeFileCoherence', () => {
    ...
    test('does not flag many small functions that share a leading name word (single domain)', async () => {
        ...
        assert.strictEqual(
            countViolation,
            undefined,
            'a shared extract_* naming domain should suppress the raw count check'
        );
    });
});
...
suite('analyzeMatchOpportunities', () => {
    ...
    test('Python: does not flag branches keyed on different variables', async () => {
        ...
        assert.strictEqual(
            violations.length,
            0,
            'expected no match-opportunity violation for unrelated branch conditions'
        );
    });
    ...
    test('Python: respects a configured minBranches threshold', async () => {
        ...
        assert.strictEqual(
            withLoweredThreshold.length,
            1,
            'expected a 2-way chain to be flagged once minBranches is lowered to 2'
        );
    });
