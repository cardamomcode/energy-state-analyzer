...
let private extendsTargetNames (heritage: Node) : string list =
    let extendsClause = nodeChildren heritage |> List.tryFind (fun c -> nodeType c = "extends_clause")
...
let private implementsTargetNames (heritage: Node) : string list =
    let implementsClause = nodeChildren heritage |> List.tryFind (fun c -> nodeType c = "implements_clause")
...
let TYPESCRIPT : LanguageAdapter =
    { Id = "typescript"
      ...
      IsFunctionDefinition =
          fun node -> nodeType node = "function_declaration" || nodeType node = "method_definition"
      ...
      ExtractTypedParameter =
          fun node ->
              if nodeType node <> "required_parameter" && nodeType node <> "optional_parameter" then
                  ...
                  let typeAnnotation = nodeChildren node |> List.tryFind (fun c -> nodeType c = typeAnnotationNodeType)
                  ...
                  | Some n, Some ta ->
                      ...
                      | Some tn -> Some { Name = nodeText n; Type = nodeText tn }
                      | None -> None
                  | _ -> None
      ExtractReturnType =
          fun node ->
