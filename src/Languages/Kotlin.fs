...
let private isAnnotatedConstValMisparse (node: Node) : bool =
    ...
    | Some annotated ->
        match nodeChildren annotated |> List.tryFind (fun c -> nodeType c = "infix_expression") with
        | Some infix ->
            let identifiers = nodeChildren infix |> List.filter (fun c -> nodeType c = "identifier")
...
let private delegationSpecifierName (specifier: Node) : string option =
    let userType =
        ...
        | Some ut -> Some ut
        | None ->
            match nodeChildren specifier |> List.tryFind (fun c -> nodeType c = "constructor_invocation") with
            | Some ci -> nodeChildren ci |> List.tryFind isUserType
            | None -> None

    match userType with
    | Some ut -> nodeChildren ut |> List.tryFind (fun c -> nodeType c = "identifier") |> Option.map nodeText
    | None -> None
...
let KOTLIN : LanguageAdapter =
    { Id = "kotlin"
      ...
      ExtractTypedParameter =
          fun node ->
              if nodeType node <> "parameter" then
                  ...
                  match nodeChildren node |> List.tryFind (fun c -> nodeType c = "identifier"), nodeChildren node |> List.tryFind isUserType with
                  | Some nameNode, Some typeNode ->
                      match nodeChildren typeNode |> List.tryFind (fun c -> nodeType c = "identifier") with
                      | Some ti -> Some { Name = nodeText nameNode; Type = nodeText ti }
                      | None -> None
                  | _ -> None
      ExtractReturnType =
          fun node ->
      ...
      PrimitiveTypeNames =
          Set.ofList [ "Int"; "Long"; "Short"; "Byte"; "Double"; "Float"; "Boolean"; "String"; "Char" ]
      ...
      GetEqualityComparisons =
          fun node ->
              if nodeType node <> "binary_expression" then
                  ...
                  match nodeChildren node |> List.tryFind (fun c -> nodeType c = "==" || nodeType c = "===") with
                  | Some opToken ->
              ...
              | Some parent when nodeType parent = "function_value_parameters" ->
                  ...
                  | Some index when index >= 2 ->
                      ...
                      | Some prev, Some prevPrev ->
                          nodeType prev = "=" && nodeType prevPrev = "parameter"
                      | _ -> false
                  | _ -> false
              | _ -> false
      ...
      GetBaseClassNames =
          fun node ->
              match nodeChildren node |> List.tryFind (fun c -> nodeType c = "delegation_specifiers") with
              | Some specifiers ->
